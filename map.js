...
function buildPopup(ecole) {
  return (
    ...
    (circonscriptionCouleurs[ecole.circonscription] || "#888") +
...
function basculerOnglet(onglet) {
  ongletActif = onglet;
  document
    .getElementById("tab-distances")
    .classList.toggle("actif", onglet === "distances");
  document
    .getElementById("tab-circo")
    .classList.toggle("actif", onglet === "circonscriptions");
  if (onglet === "circonscriptions") {
    dessinerPolygones();
  } else {
    effacerPolygones();
  }
...
function appliquerCouleursMarqueurs() {
  ecoleMarkers.forEach(function (em) {
    ...
    if (ongletActif === "circonscriptions") {
      em.marker.setStyle({
        fillColor: circonscriptionCouleurs[em.ecole.circonscription] || "#888",
      });
    } else {
...
document.getElementById("tab-distances").addEventListener("click", function () {
  basculerOnglet("distances");
});
document.getElementById("tab-circo").addEventListener("click", function () {
  basculerOnglet("circonscriptions");
});
...
function rafraichirListeCirconscriptions() {
  ...
  var stats = Object.keys(groupes).map(function (nom) {
    ...
    var avecDuree = membres.filter(function (em) {
      return em.dureeMin !== null;
    });
    var min = null,
      max = null,
    ...
    if (avecDuree.length > 0) {
      var durees = avecDuree.map(function (em) {
        return em.dureeMin;
      });
      min = Math.min.apply(null, durees);
      max = Math.max.apply(null, durees);
      moy = Math.round(
        durees.reduce(function (a, b) {
          return a + b;
        }, 0) / durees.length,
      );
    }
    return {
      nom: nom,
      couleur: circonscriptionCouleurs[nom] || "#888",
  ...
  var avecMoy = stats.filter(function (s) {
    return s.moy !== null;
  });
  var sansMoy = stats.filter(function (s) {
    return s.moy === null;
  });
  avecMoy.sort(function (a, b) {
    return a.moy - b.moy;
  });
  sansMoy.sort(function (a, b) {
  ...
  for (var i = 0; i < tries.length; i++) {
    ...
    var statsHtml =
      s.moy !== null
        ? '<span class="circo-moy">' +
        s.moy +
        " min moy.</span>" +
        '<span class="circo-range">↓ ' +
        s.min +
        " min  ↑ " +
        s.max +
        " min</span>"
    ...
    html +=
      '<div class="liste-item circo-item" data-circo="' +
  ...
  liste.querySelectorAll(".circo-item").forEach(function (item) {
    item.addEventListener("click", function () {
      ...
      var membres = ecoleMarkers.filter(function (em) {
        return em.ecole.circonscription === nom && em.visible;
      });
      if (!membres.length) return;
      var lats = membres.map(function (em) {
        return em.ecole.lat;
      });
      var lngs = membres.map(function (em) {
        return em.ecole.lng;
      });
      map.fitBounds(
        [
      ...
      liste.querySelectorAll(".circo-item").forEach(function (el) {
...
function bindClicsListe() {
  document
    .querySelectorAll(".liste-item:not(.circo-item)")
...
function mettreEnSurbrillanceListe(lat, lng) {
  document
    ...
    .forEach(function (el) {
      el.classList.toggle(
        "actif",
        parseFloat(el.dataset.lat) === lat &&
        parseFloat(el.dataset.lng) === lng,
      );
    });
